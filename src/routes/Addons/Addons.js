...
const { usePlatform, useBinaryState, withCoreSuspender } = require('rukautv/common');
const { AddonDetailsModal, Button, Image, MainNavBars, ModalDialog, SearchBar, SharePrompt, TextInput, MultiselectMenu } = require('rukautv/components');
...
const XtreamModal = require('./XtreamModal/XtreamModal');
...
const Addons = () => {
    ...
    const [xtreamModalOpen, openXtreamModal, closeXtreamModal] = useBinaryState(false);
    ...
    const onXtreamInstallSuccess = React.useCallback((result) => {
        if (result && result.addonDescriptor) {
            core.transport.dispatch({
                action: 'Ctx',
                args: {
                    action: 'InstallAddon',
                    args: result.addonDescriptor
                }
            });
        }
        toast.show({
            type: 'success',
            title: `✓ IPTV conectado: ${result.source.name}`,
            timeout: 5000
        });
    }, [core, toast]);
        ...
        <MainNavBars className={styles['addons-container']} route={'addons'}>
            <div className={styles['addons-content']}>
                <div className={styles['selectable-inputs-container']}>
                    ...
                    <Button className={styles['iptv-button-container']} title={'Agregar IPTV XC'} onClick={openXtreamModal}>
                        <Icon className={styles['icon']} name={'channels'} />
                        <div className={styles['add-button-label']}>{'IPTV XC'}</div>
                    </Button>
            ...
            }
            {
                xtreamModalOpen ?
                    <XtreamModal
                        onClose={closeXtreamModal}
                        onInstallSuccess={onXtreamInstallSuccess}
                    />
                    :
                    null
            }
